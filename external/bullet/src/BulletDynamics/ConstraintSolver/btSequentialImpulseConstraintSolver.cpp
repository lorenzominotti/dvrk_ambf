																 ...
																 const btVector3& rel_pos1, const btVector3& rel_pos2)
{
...

	{
		...
		using namespace std;
        btScalar efA1 = externalForceImpulseA[0];
        btScalar efA2 = externalForceImpulseA[1];
        btScalar efA3 = externalForceImpulseA[2];
        //cout << "\nexternalForceImpulseA    " << efA1 << "   "  << efA2 << "   "  << efA3 << "   "  << "\n";
        btScalar efB1 = externalForceImpulseB[0];
        btScalar efB2 = externalForceImpulseB[1];
        btScalar efB3 = externalForceImpulseB[2];
        //cout << "\nexternalForceImpulseB    " << efB1 << "   "  << efB2 << "   "  << efB3 << "   "  << "\n";
...
void btSequentialImpulseConstraintSolver::writeBackContacts(int iBegin, int iEnd, const btContactSolverInfo& infoGlobal)
{
	for (int j = iBegin; j < iEnd; j++)
	{
		...
		using namespace std;
        // << "\n\nAPPLIED IMPULSE      " << pt->m_appliedImpulse << "\n\n\n";
		//cout << "\n\nAPPLIED IMPULSE      " << pt->m_appliedImpulse/infoGlobal.m_timeStep << "\n\n\n";
...
void btSequentialImpulseConstraintSolver::writeBackJoints(int iBegin, int iEnd, const btContactSolverInfo& infoGlobal)
{
	for (int j = iBegin; j < iEnd; j++)
	{
		...
		if (fb)
		{
			...
			/*std::cout << "\n\n\n\nm_appliedForceBodyA     " << fb->m_appliedForceBodyA[0] << "   " << fb->m_appliedForceBodyA[1] << "   " << fb->m_appliedForceBodyA[2];
            std::cout << "\n\nm_appliedForceBodyB     " << fb->m_appliedForceBodyB[0] << "   " << fb->m_appliedForceBodyB[1] << "   " << fb->m_appliedForceBodyB[2];
            std::cout << "\n\nm_appliedTorqueBodyA    " << fb->m_appliedTorqueBodyA[0] << "   " << fb->m_appliedTorqueBodyA[1] << "   " << fb->m_appliedTorqueBodyA[2];
            std::cout << "\n\nm_appliedTorqueBodyB    " << fb->m_appliedTorqueBodyB[0] << "   " << fb->m_appliedTorqueBodyB[1] << "   " << fb->m_appliedTorqueBodyB[2];*/
		}
