...
class Insertion:
	...
	def plot(self):
		#np.savetxt('ambf/ambf_ros_modules/ambf_comm/scripts/tests_ambf/01NewCode/test_ins/sp_ins_f.csv', self.graph_f, delimiter=",")
		#np.savetxt('ambf/ambf_ros_modules/ambf_comm/scripts/tests_ambf/01NewCode/test_ins/sp_ins_t.csv', self.time, delimiter=",")
...
def main():
	...
	ins = Insertion()
	ins.reachFhigh(m_start, 2, True)
	ins.reachFlow(m_start, 0.5, False)
	ins.reachFhigh(m_start, 2, False)
	ins.reachFlow(m_start, 0.5, False)
	ins.plot()
