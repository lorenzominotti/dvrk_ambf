...
class Cartesian_control:
	...
	def approach_goal_Z(self, m_start):
		...
		while self.m < self.limit_mi:
			...
			self.graph_pz = np.append(self.graph_pz,-0.15325)
			PID = 1
			...
			self.pz = np.append(self.pz, -0.15325)
			...
			self.graph_pzd = np.append(self.graph_pzd, -0.15325) #originally 0.2045
	...
	def plot_force(self):
		...
		#np.savetxt('ambf/ambf_ros_modules/ambf_comm/scripts/tests_ambf/01NewCode/test_plots/joints/05_rb_cart_error.csv', self.er, delimiter=",")
