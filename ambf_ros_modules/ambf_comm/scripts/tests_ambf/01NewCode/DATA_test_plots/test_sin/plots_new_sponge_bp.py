...
error_sp_05 = np.loadtxt('ambf/ambf_ros_modules/ambf_comm/scripts/tests_ambf/01NewCode/test_plots/test_sin/05_sp_sin_error.csv', delimiter = ",")
time_sp_05 = np.loadtxt('ambf/ambf_ros_modules/ambf_comm/scripts/tests_ambf/01NewCode/test_plots/test_sin/05_sp_sin_time.csv', delimiter = ",")
...
minlen = len(error_sp_01)
if len(error_sp_02) < minlen:
	minlen = len(error_sp_02)
if len(error_sp_03) < minlen:
	minlen = len(error_sp_03)
if len(error_sp_04) < minlen:
	minlen = len(error_sp_04)
if len(error_sp_05) < minlen:
	minlen = len(error_sp_05)


error_sp_01 = error_sp_01[0:minlen]
error_sp_02 = error_sp_02[0:minlen]
error_sp_03 = error_sp_03[0:minlen]
error_sp_04 = error_sp_04[0:minlen]
error_sp_05 = error_sp_05[0:minlen]
...
'''
plt.plot(time_sp_05, error_sp_05)
plt.grid()
plt.show()
'''
...
bp0 = axes[0].boxplot(error_sp_01, patch_artist=True, showfliers=False)
...
bp1 = axes[1].boxplot(error_sp_02, patch_artist=True, showfliers=False)
...
bp2 = axes[2].boxplot(error_sp_03, patch_artist=True, showfliers=False)
...
bp2 = axes[3].boxplot(error_sp_04, patch_artist=True, showfliers=False)
for box in bp2['boxes']:
    # change outline color
    box.set( color='k', linewidth=2)
    # change fill color
...
bp2 = axes[4].boxplot(error_sp_05, patch_artist=True, showfliers=False)
for box in bp2['boxes']:
    # change outline color
    box.set( color='k', linewidth=2)
    # change fill color
