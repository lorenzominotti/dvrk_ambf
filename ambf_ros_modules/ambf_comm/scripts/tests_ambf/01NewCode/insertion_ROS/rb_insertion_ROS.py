...
pub_irb = rospy.Publisher('ins_rb', Float64, queue_size=10)
pub_t = rospy.Publisher('time_rb', Float64, queue_size=10)

def publish_to_plot(force,time):

		pub_irb.publish(force)
		pub_t.publish(deltat_a)
...
while m < limit_mi:
	...
	#print("wait", wait) 
	#print("delta",time.time()-time_start_a)
	#print("cycle-force", time.time()-time_start_a, force)


	if (wait)>0:
		time.sleep(wait)
		#print("WEEEEEEEE")
	publish_to_plot(force,time1)

	hold = time.time()-time_start_a
	print("cycle-force", time.time()-time_start_a, force)
...
plt.figure()
plt.plot(time1, graph_f, color = 'r', label = "Force")
plt.xlabel('Time [s]')
plt.ylabel('Force [N]')	
plt.legend(loc='best')
plt.grid()
