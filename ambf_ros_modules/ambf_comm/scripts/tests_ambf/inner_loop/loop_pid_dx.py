	...
	def reach_pos_XY(self, goal_x, goal_y, start):
		...
		while (stop_x == False) or (stop_y == False):
			...
			if self.flag_first_pos == False:
				...
				self.Integratorx = self.Integratorx + delta_cart[0]
				self.I_value = self.Integratorx * Kix		
				PID = self.P_value + self.I_value 
				delta_cart[0] = delta_cart[0] + PID*delta_cart[0]

				Kpy = 0.9
				Kiy = 0.2
				self.P_value = (Kpy * delta_cart[1])		
				self.Integratory = self.Integratory + delta_cart[1]
				self.I_value = self.Integratory * Kiy		
				PID = self.P_value + self.I_value 
				delta_cart[1] = delta_cart[1] + PID*delta_cart[1]
				
				Kpz = 0.005
				Kiz = 0.0008	
				self.P_value = (Kpz * delta_cart[2])		
				self.Integratorz = self.Integratorz + delta_cart[2]
				self.I_value = self.Integratorz * Kiz		
				PID = self.P_value + self.I_value 
				delta_cart[2] = delta_cart[2] + PID*delta_cart[2]
...
def main():
	...
	#cart_c.reach_pos_XY(0.06, 0.06, True)
