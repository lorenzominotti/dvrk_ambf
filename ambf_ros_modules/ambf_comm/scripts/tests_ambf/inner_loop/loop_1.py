...
class Cartesian_control:
	...
	def reach_pos_XY(self, goal_x, goal_y, start):
		...
		while (stop_x == False) or (stop_y == False):
			...
			X_desired_l = X_desired
			Y_desired_l = Y_desired
			Z_desired_l = Z_desired
			i = 0
			while i<10:
				Kpx = 0.9
				Kix = 0.02	
				...
				Kpz = 0.005
				Kiz = 0.000001
				errorz = self.Z_desired_2 - z_loop
				self.P_value = (Kpz * errorz)		
				self.Integrator = self.Integrator + errorz
				self.I_value = self.Integrator * Kiz		
				PID = self.P_value + self.I_value 
				Z_desired_2 = self.Z_desired_2 + PID*self.Z_desired_2

				X_desired_l = X_desired_2
				Y_desired_l = Y_desired_2
				Z_desired_l = Z_desired_2
				i = i + 1
			...
			self.graph_pz = np.append(self.graph_pz, self.Z_desired_2)
