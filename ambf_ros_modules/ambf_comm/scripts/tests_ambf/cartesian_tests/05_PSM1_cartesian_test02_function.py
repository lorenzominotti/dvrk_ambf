...
class Cartesian_control:
	...
	def reach_pos_XY0(self, goal_x, goal_y, start):
	...
	flag_first_pos = True
	def test_pos(self, x_des, y_des, z_des):
		
		
		self.x_plot = np.append(self.x_plot, x_des)
		self.y_plot = np.append(self.y_plot, y_des)
		self.z_plot = np.append(self.z_plot, z_des)

		self.inverse_kinematics(x_des, y_des, z_des)
		self.jacobian(self.q1, self.q2, self.q3)
		...
		if self.flag_first_pos == True:
			...
			self.get_position_joints_PSM()
			self.forward_kinematics(self.q1_read, self.q2_read, self.q3_read)
			self.pos_actualx = np.append(self.pos_actualx, self.q1_read)
			self.pos_actualy = np.append(self.pos_actualy, self.q2_read)
			self.pos_actualz = np.append(self.pos_actualz, self.q3_read)
		self.flag_first_pos = False
		...
		if self.flag_first_pos == False:
			...
			delta_cart = [0, 0, 0]
			delta_q = [0, 0, 0]
			delta_cart[0] = x_des-self.x_fk
			delta_cart[1] = y_des-self.y_fk
			delta_cart[2] = z_des-self.z_fk
			#print(delta_cart)
			invJ = np.linalg.inv(self.jac) 
			delta_q = invJ.dot(delta_cart)
			#print(delta_q)
			...
			self.pos_actualy = np.append(self.pos_actualy, self.q2_read)
			self.pos_actualz = np.append(self.pos_actualz, self.q3_read)
			print(z_des, self.q3_read)
		
	def test_pos0(self, x_des, y_des, z_des):
		...
		a[0][0].plot(self.x_plot)
		a[0][0].plot(self.pos_actualx)
		a[0][0].grid()
		
		#plt.figure(2)
		a[1][0].plot(self.y_plot)
		a[1][0].plot(self.pos_actualy)
		a[1][0].grid()
		
		#plt.figure(3)
		a[0][1].plot(self.z_plot)
		a[0][1].plot(self.pos_actualz)
		a[0][1].grid()
