...
degree = 0
while degree < 30:
 psm_handle_base.set_joint_pos(0, math.radians(degree))
 time.sleep(0.1)
 degree = degree + 0.1
 '''
...
target_x = posx_start - 0.12
target_y = posy_start + 0.05
print('target_x')
print(target_x)   
print('\n')
print('target_y')
...
while (stop_x == False) or (stop_y == False):
	...
	if target_x >= posx_start:
		if target_x > px:
			psm_handle_base.set_joint_pos(0, math.radians(-dx*step))
		else:
			stop_x = True
	if target_x < posx_start:
		if target_x < px:
	...
	if target_y >= posy_start:
		if target_y > py:
			psm_handle_pfl.set_joint_pos(0, math.radians(dy*step))
		else:
			stop_y = True
	if target_y < posy_start:
		if target_y < py:
